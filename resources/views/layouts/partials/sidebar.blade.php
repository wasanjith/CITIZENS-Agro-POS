@php
    ...
    $showReports = app(\App\Domain\Reports\ReportRegistry::class)->anyFor(auth()->user());
    ...
    $sections = [
        ...
        'Administration' => ['icon' => 'cog', 'items' => [
            ...
            ['label' => 'Go-live', 'route' => 'admin.go-live.index', 'active' => 'admin.go-live.*', 'can' => 'admin.settings.manage'],
        ]],
    ];
@endphp
...
<nav class="flex min-h-0 flex-1 flex-col gap-1 overflow-y-auto px-3 py-4 sidebar-scroll" aria-label="Main">
    ...
    @if ($showReports)
        <a
            ...
            @class([
                'flex items-center gap-3 rounded-md px-3 py-2 text-sm font-semibold',
                'bg-brand-800 text-white' => $reportsActive,
                'text-brand-100 hover:bg-brand-800/60 hover:text-white' => ! $reportsActive,
            ])
            @if ($reportsActive) aria-current="page" @endif
        >
            <svg class="size-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true">
                @foreach ($icons['chart'] as $d)<path stroke-linecap="round" stroke-linejoin="round" d="{{ $d }}" />@endforeach
            </svg>
            Reports
        </a>
    @endif
